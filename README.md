...
3.  **Исполнение:** 
       1. **TASK** — воспроизведение видео

     Агент загружает видеофайл из рабочего репозитория и открывает его системным проигрывателем.
    
    Параметр options может быть передан в одном из форматов:
    
    | Формат | Пример | Результат |
    | :--- | :--- | :--- |
    | Число  |	2  | Видео screamer2.mp4|
    | Строка с именем файла | "screamer2.mp4" |	Указанный файл |
    
    → Агент загрузит и откроет screamer2.mp4
    
       2. **FILE** — отправка результатов

     Агент отправляет на сервер содержимое папки results_folder и свой лог-файл.
    
    Параметр options не нужен (игнорируется).
    
       3. **CONF** — остановка агента
    
    Агент завершает свою работу.
    
    Параметр options не нужен.
    
       4. **TIMEOUT** — изменение частоты опроса
    
    Агент меняет интервал между запросами к серверу.
    
    Параметр options — строка с новым интервалом в секундах.
    ...
    ```
    → Агент будет опрашивать сервер каждые 30 секунд
